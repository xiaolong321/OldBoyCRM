/**
 * Created by xuebaoku on 16/5/18.
 */
 ...
 * 添加 本地消息
 */
function add_message_mini(mode,size,title,msg, delay) {
    Lobibox.notify(mode, {
        size: size,
        delay: 1000,
        title: title,
...
function add_message(event) {
    var parsedJson = jQuery.parseJSON(event);
    $.each(parsedJson,function(key,value){
        var fields = value.fields;
        var Message = jQuery.parseJSON(fields.Message);
        // console.info(Message);
        if(Message.models == 'popup'){
            Lobibox.alert(
                Message.type,
                {
                    title: Message.title,
...
function get_message(data) {
    // 拉取 消息内容
    var Url = '/auth/message/';
    data['action_name'] = 'get_message';
    // onsole.info(data);
    $.AdminLTE.apiGet(
         Url,
         data,
         function (resp) {
             // console.info(resp);
             if(resp.ret_code != 0){
                 add_message_mini('warning', 'mini', '连接错误', '短信消息连接出错.')
             }else{
                 add_message(resp.results)
             }
         }
    );
}

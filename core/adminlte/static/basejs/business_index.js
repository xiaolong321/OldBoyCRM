...
function AggregationSearchCondition(conditions){
    ...
    if(value) {
        ...
        ret['contact__name__contains'] = ret[name];
        ret['user_group__name__contains'] = ret[name];
    }
    return ret;
}
...
function InitGlobalDict(callback){
    window.window_contact = callback.contact_choice.data;
    window.window_user_group = callback.user_group_choice.data;
}
...
function EmbedIntoTable(response,startNum,body){
    if(!response.status){
        ...
        $.each(response.data,function(key,value){
            ...
            tds.push($.CreateTd({'edit-enable':'true','edit-type':'select','value_key':'id','text_key':'name','name':'contact_id','origin':value.contact__id,'edit-option':'contact','options':'window_contact'},{}, value.contact__name));
            tds.push($.CreateTd({'edit-enable':'true','edit-type':'select','value_key':'id','text_key':'name','name':'user_group_id','origin':value.user_group__id,'edit-option':'user_group','options':'window_user_group'},{}, value.user_group__name));
            tds.push($.CreateTd({'edit-enable':'false','edit-type':'input','name':'asset_count','origin':value.ip_count},{}, value.ip_count));

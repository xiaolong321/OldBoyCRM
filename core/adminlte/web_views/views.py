...
from ..web_models.models import SystemConfig
from ..web_models import constants
...
def get_system_config_value(key_name, status=1, **kwargs):
    try:
        return SystemConfig.objects.get(name=key_name, status=status, **kwargs).value
    except:
...
class CommonListPageView(CommonPageViewMixin, ListView):
    ...
    def get_table_titles(self):
        ...
        :return:
        """
        def get_test_fieldes(name,meta_fields,model,titles):
            meta_names = [mf.name for mf in meta_fields]
            if type(name) == dict:
                titles.append(name['name'])
                show_fields.append(name['show_fields'])
                return

            show_fields.append(name)
            if name not in meta_names:
                try:
                    t = getattr(model, name).field.verbose_name
                    titles.append(t)
        ...
        show_field = self.model.Config.list_display_fields
        meta_field = self.model._meta.fields
        titles = []
        show_fields = []
        for name in show_field:
            get_test_fieldes(name=name,model=self.model,meta_fields=meta_field,titles=titles)

...
def generate_chart_data(survery_obj):
    ...
    sum_list = models.SurveryRecord.objects.filter(survery_id=survery_obj.id,survery_item__anwser_type='score').values("survery__id","survery_item__name","survery_item_id").annotate(score_sum=Sum("score"),survery_count=Count('score'))

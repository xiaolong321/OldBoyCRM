...
urlpatterns = [
    ...
    url(r'^tracking-(?P<source>\S+)-(?P<course>\S+)-(?P<class_type>\S+)-(?P<status>\S+)-(?P<filter_date>\S+)-(?P<consultant__email>\S+)-(?P<network_consultant__email>\S+).html/(?P<page>\d*)',
        views.tracking, name='tracking'),  # 跟踪用户
    url(
        r'^signed-(?P<source>\S+)-(?P<course>\S+)-(?P<class_type>\S+)-(?P<status>\S+)-(?P<filter_date>\S+)-(?P<consultant__email>\S+)-(?P<network_consultant__email>\S+).html/(?P<page>\d*)',
        views.signed, name='signed'),  # 已签约客户
    url(
        r'^customers_library-(?P<source>\S+)-(?P<course>\S+)-(?P<class_type>\S+)-(?P<status>\S+)-(?P<filter_date>\S+)-(?P<consultant__email>\S+)-(?P<network_consultant__email>\S+).html/(?P<page>\d*)',
    ...
    url(r'^pushcustomer/$', views.pushcustomer, name='pushcustomer'),
    url(r'^add_referral_customer/(?P<referralfromid>\d+)/$', views.addcustomer, name='add_referral_customer'),
    url(r'^add_push_customer/(?P<pushid>\d+)/$', views.addcustomer, name='add_push_customer'),
    ...
    url(r'^class_list-(?P<course>\S+)-(?P<semester>\S+)-(?P<start_date>\S+)-(?P<graduate_date>\S+)-(?P<teachers__email>\S+).html', views.class_list, name='class_list'),
